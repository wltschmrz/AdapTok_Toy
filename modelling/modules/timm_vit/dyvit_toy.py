...
class Attention(nn.Module):
    ...
    def forward(self, x, policy,
                freqs_cis=None, num_prefix_tokens=1, num_latent_tokens=32):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)

        # Apply RoPE if provided (skip prefix, optionally skip trailing latent tokens)
        if freqs_cis is not None:
            # q,k shape: (B, H, N, head_dim)
            start = int(num_prefix_tokens)
            end = N if int(num_latent_tokens) == 0 else (N - int(num_latent_tokens))
            if end > start:  # 유효 구간이 있을 때만 적용
                q_head = q[:, :, start:end, :]
                k_head = k[:, :, start:end, :]
                q_rot, k_rot = apply_rotary_emb(q_head, k_head, freqs_cis=freqs_cis)
                q = torch.cat([q[:, :, :start, :], q_rot, q[:, :, end:, :]], dim=2)
                k = torch.cat([k[:, :, :start, :], k_rot, k[:, :, end:, :]], dim=2)
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ...
    if freqs_cis.shape == (x.shape[-2], x.shape[-1]):
        ...
        raise ValueError(
            f"freqs_cis shape {freqs_cis.shape} not compatible with x shape {x.shape} "
            "(expected (N, D2) or (H, N, D2))"
        )
...
def apply_rotary_emb(xq: torch.Tensor, xk: torch.Tensor, freqs_cis: torch.Tensor):
    ...
    D = xq.shape[-1]
    assert D % 2 == 0, "last dim must be even for complex packing"

    xq_c = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], D // 2, 2))
    xk_c = torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], D // 2, 2))

    # broadcast freqs
    if not torch.is_complex(freqs_cis):
        raise TypeError("freqs_cis must be a complex tensor (e^{iθ}).")
    freqs_c = reshape_for_broadcast(freqs_cis, xq_c).to(device=xq_c.device, dtype=xq_c.dtype)

    # rotate
    xq_rot = xq_c * freqs_c
    xk_rot = xk_c * freqs_c

    # back to real, restore original shape & dtype/device
    xq_out = torch.view_as_real(xq_rot).reshape(*xq.shape).type_as(xq)
    xk_out = torch.view_as_real(xk_rot).reshape(*xk.shape).type_as(xk)
...
class Block(nn.Module):
    ...
    def forward(self, x, policy=None, 
        ...
        x = x + self.drop_path(self.attn(self.norm1(x), policy=policy, 
                                         freqs_cis=freqs_cis, num_prefix_tokens=num_prefix_tokens, 
...
#         return self.out_conv(x)  # (BN2)

# NEW
class PrefixRouter(nn.Module):
    def __init__(self, embed_dim, down_scale=2):
        super().__init__()
        self.in_conv = nn.Sequential(
            nn.LayerNorm(embed_dim),
            nn.Linear(embed_dim, embed_dim),
        ...
        self.scorer = nn.Sequential(
            ...
            nn.Linear(2 * embed_dim, embed_dim // int(down_scale / 2)),
            nn.GELU(),
            nn.Linear(embed_dim // int(down_scale / 2), embed_dim // int(2*down_scale / 2)),
            nn.GELU(),
...
class VisionTransformerDiffPruning(nn.Module):
    def __init__(self, 
                 img_size=128, patch_size=16, in_chans=3, num_classes=1000, embed_dim=192, depth=8,
                 num_heads=2, mlp_ratio=4., qkv_bias=True, qk_scale=None, representation_size=None,
        ...
        # NEW: 단일 인덱스 선택 router  #####
        router_list = [PrefixRouter(embed_dim) for _ in range(len(pruning_loc))]
        self.score_predictor = nn.ModuleList(router_list)

        # NEW: routing 하이퍼  #####
        self.router_tau = 1.0     # softmax temperature
        self.use_ste = True
